...
require "#{File.dirname(__FILE__)}/resources/NCFaultCalculationChillerElectricEIR"
...
# start the measure
class ChillerNonCondensable < OpenStudio::Ruleset::WorkspaceUserScript

  # human readable name
  def name
    return "Water-cooled chiller operating with non-condensable and refrigerant mixture"
  end

  # human readable description
  def description
    return "This Measure simulates the effect of non-condensable circulating in water-cooled chillers with shell-and-tube condensers and evaporators to the building performance."
  end

  # human readable description of workspace approach
  def modeler_description
    return "To use this Measure, choose the Chiller:Electric:EIR object to be faulted and a schedule of fault level. Define the fault level as the ratio of the volume inside the refrigerant circuit occupied by the non-condensable to the total volume of the refrigerant circuit when the system in not operating at standard condition. If the fault level is outside the range of zero and one, an error will occur."
  end
  ...
  def arguments(workspace)
    ...
    fault_level.setDisplayName("Non-condensable level of the Chiller:Electric:EIR object. This model only simulates overcharged condition so the number should be between 0 and 1.")
    fault_level.setDefaultValue(0.03)  #default fouling level to be 10%
    args << fault_level
    
    #fault level limits
    max_fl = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("max_fl", true)
    max_fl.setDisplayName("Maximum value of fault level")
    max_fl.setDefaultValue(0.05)
    args << max_fl
    
    #non-condensable model
    args = enter_coefficients(args, $power_para_num, "power_fault", [6.857380961, -0.038358561, 0.015675754, 3.285341992, -2.374185964], " for the non-condensable model")
        
    min_evap_tmp_fault = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("min_evap_tmp_fault", true)
    min_evap_tmp_fault.setDisplayName("Minimum value of evaporator water outlet temperature for the non-condensable model (C)")
    min_evap_tmp_fault.setDefaultValue(4.4)  #the first number is observed from the training data, and the second number is an adjustment for range
    args << min_evap_tmp_fault
    
    max_evap_tmp_fault = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("max_evap_tmp_fault", true)
    max_evap_tmp_fault.setDisplayName("Maximum value of evaporator water outlet temperature for the non-condensable model (C)")
    max_evap_tmp_fault.setDefaultValue(10.3)
    args << max_evap_tmp_fault
    
    min_cond_tmp_fault = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("min_cond_tmp_fault", true)
    min_cond_tmp_fault.setDisplayName("Minimum value of condenser inlet temperature for the non-condensable model (C)")
    min_cond_tmp_fault.setDefaultValue(17.4)
    args << min_cond_tmp_fault
    
    max_cond_tmp_fault = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("max_cond_tmp_fault", true)
    max_cond_tmp_fault.setDisplayName("Maximum value of condenser inlet temperature for the non-condensable model (C)")
    max_cond_tmp_fault.setDefaultValue(29.9)
    args << max_cond_tmp_fault
    
    min_cap_fault = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("min_cap_fault", true)
    min_cap_fault.setDisplayName("Minimum ratio of evaporator heat transfer rate to the reference capacity for the non-condensable model (kW)")
    min_cap_fault.setDefaultValue(0.27)
    args << min_cap_fault
    
    max_cap_fault = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("max_cap_fault", true)
    max_cap_fault.setDisplayName("Maximum value of reference capacity for the non-condensable model (kW)")
  ...
  def run(workspace, runner, user_arguments)
    ...
    # if (schedule_exist || true) # only continue if the user is running the module
      ...
      if no_RTU_changed
        runner.registerError("Measure ChillerNonCondensable cannot find "+chiller_choice+". Exiting......")
      ...
      runner.registerAsNotApplicable("ChillerNonCondensable is not running for "+chiller_choice+". Skipping......")
    end
...
ChillerNonCondensable.new.registerWithApplication

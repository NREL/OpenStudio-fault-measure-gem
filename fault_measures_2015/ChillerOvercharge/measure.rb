...
# start the measure
class ChillerOvercharge < OpenStudio::Ruleset::WorkspaceUserScript
  # human readable name
  def name
    return 'Water-cooled chiller overcharged'
  end

  # human readable description
  def description
    return 'This Measure simulates the effect of overcharging of water-cooled chillers with shell-and-tube condensers and evaporators to the building performance.'
  end
  ...
  def arguments(workspace)
    ...
    max_fl.setDefaultValue(1.4) # default maximum level to be overcharged by 40%
    args << max_fl

    # overcharging model
    args = enter_coefficients(args, $power_para_num, 'power_fault', [-6.52908026, 0.004629978, 0.016836438, 2.442640545, -1.525441444], ' for the overcharging model')
    ...
    min_evap_tmp_fault.setDefaultValue(4.04)
    ...
    min_cond_tmp_fault.setDefaultValue(17.9)
  ...
  def run(workspace, runner, user_arguments)
    ...
    runner.registerAsNotApplicable("ChillerOvercharge is not running for #{chiller_choice}. Skipping......")
    return true
  end
  ...
  def check_schedule_or_lvl_error(workspace, runner, sch_choice, schedule_exist, fault_level)
    ...
    if schedule_exist
      ...
      if fault_level < 1.0
        runner.registerError("Fault level #{fault_level} in ChillerOvercharge is lower than 1.0. Exiting......")
        return scheduletypelimits, true
      end
    end
  ...
  def error_msg_for_not_rtu(runner, chiller_choice, existing_coils)
    ...
    runner.registerError("Measure ChillerOvercharge cannot find #{chiller_choice}. Exiting......")
...
ChillerOvercharge.new.registerWithApplication

...
At some point the OSM could become the working model that gets updated and converted to IDF as needed. In that case this meaure be needed. There will be a mostly complete OSM model that just uses measure to adapt operational characteristics as needed."
  ...
  def arguments(model)
    ...
    # argument for infil_flow_ext_wall_area
    infil_flow_ext_wall_area = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("infil_flow_ext_wall_area",true)
    infil_flow_ext_wall_area.setDisplayName("Infiltration Flow per Exterior Wall Area")
    infil_flow_ext_wall_area.setUnits("CFM/ft^2")
    infil_flow_ext_wall_area.setDescription("Used for all Zones but Room 102 and Room 101 which uses ACH")
    infil_flow_ext_wall_area.setDefaultValue(0.2)
    args << infil_flow_ext_wall_area

    # argument for infil_102ach
    infil_102ach = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("infil_102ach",true)
    infil_102ach.setDisplayName("Air Changes per Hour for Room 102")
    infil_102ach.setUnits("ACH")
    infil_102ach.setDescription("Only affects Room 101 abd 102 which have exterior doors")
    infil_102ach.setDefaultValue(2.0)
    args << infil_102ach

    # argument for stair_doors_mixing
    stair_doors_mixing = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("stair_doors_mixing",true)
    stair_doors_mixing.setDisplayName("Air Changes per Mixing Between Stair doors and adjacent internal spaces")
    stair_doors_mixing.setUnits("CFM")
    stair_doors_mixing.setDescription("Value is added between both space 101 and 102, and 201 and 202")
    stair_doors_mixing.setDefaultValue(0.5)
    args << stair_doors_mixing
  ...
  def run(model, runner, user_arguments)
    ...
    infil_flow_ext_wall_area = runner.getDoubleArgumentValue("infil_flow_ext_wall_area",user_arguments)
    infil_102ach = runner.getDoubleArgumentValue("infil_102ach",user_arguments)
    stair_doors_mixing = runner.getDoubleArgumentValue("stair_doors_mixing",user_arguments)
    ...
    model.getSpaces.each do |space|
      ...
      if ["Room 101","Room 102"].include?(space.name.get.to_s)
        infil.setAirChangesperHour(infil_102ach)
        runner.registerInfo("Setting infiltration to #{infil_102ach} ACH for #{space.name}.")
      else
        infil_flow_ext_wall_area_si = OpenStudio.convert(infil_flow_ext_wall_area,"cfm/ft^2","m/s").get
        infil.setFlowperExteriorWallArea(infil_flow_ext_wall_area_si)
        runner.registerInfo("Setting infiltration to #{infil_flow_ext_wall_area} CFM per ft^2 of exterior wall for #{space.name}.")
      end
...
=end
    ...
    # alter envelope construction characteristics
    ...
    # get zones for mixing
    zone_101 = model.getThermalZoneByName('Room 101').get
    zone_102 = model.getThermalZoneByName('Room 102').get
    zone_201 = model.getThermalZoneByName('Room 201').get
    zone_202 = model.getThermalZoneByName('Room 202').get
    target_zone_mixing_si = OpenStudio.convert(stair_doors_mixing,"cfm","m^3/s").get
    runner.registerInfo("Setting zone mixing to/from zones with doors to unconditioned stair with value of #{stair_doors_mixing} (cfm).")

    # add zone mixing between 101 and 102, and 201 and 202
    zone_mixing = OpenStudio::Model::ZoneMixing.new(zone_101)
    zone_mixing.setSourceZone(zone_102)
    zone_mixing.setAirChangesperHour(target_zone_mixing_si)
    zone_mixing.setDeltaTemperature(0.0)
    zone_mixing = OpenStudio::Model::ZoneMixing.new(zone_102)
    zone_mixing.setSourceZone(zone_101)
    zone_mixing.setAirChangesperHour(target_zone_mixing_si)
    zone_mixing.setDeltaTemperature(0.0)
    zone_mixing = OpenStudio::Model::ZoneMixing.new(zone_201)
    zone_mixing.setSourceZone(zone_202)
    zone_mixing.setAirChangesperHour(target_zone_mixing_si)
    zone_mixing.setDeltaTemperature(0.0)
    zone_mixing = OpenStudio::Model::ZoneMixing.new(zone_202)
    zone_mixing.setSourceZone(zone_201)
    zone_mixing.setAirChangesperHour(target_zone_mixing_si)
    zone_mixing.setDeltaTemperature(0.0)

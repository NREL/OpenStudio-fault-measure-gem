...
class AvgBldgTemperatureTimeSeriesObjectiveFunction < OpenStudio::Ruleset::ReportingUserScript
  ...
  def run(runner, user_arguments)
    ...
    # todo - update to replace sql_key and sql_var (where is map used)
    map = {"#{csv_var}" => { key: "Conditioned Zones", var: "Zone Mean Air Temperature", index: 0 }}
      ...
      if (hdr.to_s != csv_time_header.to_s)
        ...
        environment_periods.each do |environment_p|
          ...
          model.getThermalZones.each do |zone|
            ...
            key_value =  zone.name.to_s
            output_timeseries = sqlFile.timeSeries(environment_p, reporting_frequency, "Zone Mean Air Temperature", key_value) # key value would go at the end if we used it.
            if output_timeseries.empty?
              runner.registerWarning("Timeseries not found for #{key_value}.")
            else
              # populate hash
              zone_data[key_value] = {:volume => zone.airVolume, :hourly_temp  => output_timeseries.get}
              runner.registerInfo("Found data for #{key_value} which has volume of #{zone.airVolume}")
            end
          end
          ...
          zone_data.each do |k,v|
            ...
            for i in 0..(values.size - 1)
              if building_avg_values.size <= counter
          ...
          building_avg_values.each_with_index do |value,i|
            next if i <= 222 * 24 * 4 # skip if not yet 8/10
            os_vector[i] = value
          end
          # make timeseries

...
# start the measure
class AvgBldgTemperatureTimeSeriesObjectiveFunction < OpenStudio::Ruleset::ReportingUserScript

  # human readable name
  def name
    return "Avg Bldg Temperature TimeSeries Objective Function"
  end

  # human readable description
  def description
    return "Custom version of time series objective function that is setup to report volume weighted average building air temperature"
  end
  ...
  def arguments()
    ...
    csv_var.setDefaultValue("Wtg_Avg_Cond_Zone_Temp")
    args << csv_var
    ...
    convert_data.setDefaultValue("F to C")
    args << convert_data
    ...
    sql_var.setDefaultValue("Zone Mean Air Temperature")
    args << sql_var
  ...
  def energyPlusOutputRequests(runner, user_arguments)
    ...
    runner.registerInfo("Adding variable reqeust for #{variable},#{key},#{timestep}")
  ...
  def run(runner, user_arguments)
    ...
    cvrmse = 0
    nmbe = 0
    map = {"#{csv_var}" => { key: "Conditioned Zones", var: sql_var, index: 0 }}
    ...
    csv[0].each do |hdr|
      if (hdr.to_s != csv_time_header.to_s)
        ...
        environment_periods.each do |environment_p|
          ...
          # loop through zones and gather timeseries data
          zone_data = {}
          model.getThermalZones.each do |zone|
            # skip zones that are not conditioned
            next if zone.canBePlenum
            key_zone_name =  zone.name.to_s
            output_timeseries = sqlFile.timeSeries(environment_p, reporting_frequency, sql_var, key_zone_name) # key value would go at the end if we used it.
            if output_timeseries.empty?
              runner.registerWarning("Timeseries not found for #{key_zone_name}.")
            else
              # populate hash
              zone_data[key_zone_name] = {:volume => zone.airVolume, :hourly_temp  => output_timeseries.get}
              runner.registerInfo("Found data for #{key_zone_name} with #{output_timeseries.get.values.size} items which has volume of #{zone.airVolume}")

              # temp array of values for single zone
              #array = []
              #for i in 0..(output_timeseries.get.values.size - 1)
              #  array << output_timeseries.get.values[i].round(1)
              #end
              #runner.registerInfo(array.inspect)
            end
          end
          # create volume weighted average temperature
          building_avg_values = {}
          total_volume = 0.0
          zone_data.each do |k,v|
            total_volume += v[:volume]
            values = v[:hourly_temp].values
            counter = 0
            for i in 0..(values.size - 1)
              if !building_avg_values.has_key?(counter)
                building_avg_values[counter] = values[i] * v[:volume]
              else
                building_avg_values[counter] += values[i] * v[:volume]
              end
              counter += 1
          ...
          # divide all values for building_avg_values by total volume
          #building_avg_values.map! {|item| item / total_volume}

          # convert array to vector
          os_vector = OpenStudio::Vector.new(building_avg_values.size)
          building_avg_values.each do |i,v|
            next if i.to_f <= 2 * 24 * 4 # skip if not yet 8/11
            os_vector[i] = v / total_volume
          end
          # make timeseries
          date_times = zone_data.values.first[:hourly_temp].dateTimes
          units = zone_data.values.first[:hourly_temp].units
          ser = OpenStudio::TimeSeries.new(date_times,os_vector,units)
          runner.registerInfo("Compiled volume weighted average zone air temperature for #{os_vector.size} timesteps.")
...
AvgBldgTemperatureTimeSeriesObjectiveFunction.new.registerWithApplication

...
# start the measure
class SplitCAWithSHRChange < OpenStudio::Ruleset::WorkspaceUserScript
  # human readable name
  def name
    return 'Split System Condenser Fouling Simulation'
  end

  # human readable description
  def description
    return 'This Measure simulates the effect of condenser fouling ' \
      'of any Split System to the building performance, including the sensible heat ' \
      'ratio effect..'
  end
  ...
  def arguments(workspace)
    ...
    # undercharging model
    args = enter_coefficients(args, $q_para_num, "Q_#{$faultnow}", [1.303700, -0.763560, -0.611170, -0.013457, -0.269110, 0.020726], '')
    args = enter_coefficients(args, $eir_para_num, "EIR_#{$faultnow}", [-4.586400, 5.708100, -1.516700, -0.138630, 1.218900, 0.070849], '')
    ...
    min_cop_uc.setDefaultValue(5.11)
    args << min_cop_uc
    ...
    max_cop_uc.setDefaultValue(7.06)
    args << max_cop_uc
  ...
  def run(workspace, runner, user_arguments)
    ...
    # find the Split System to change
    splitsystem_changed = false
    existing_coils = []
    ...
    coilcoolingdxsinglespeeds.each do |coilcoolingdxsinglespeed|
      ...
      splitsystem_changed = _write_ems_string(workspace, runner, user_arguments, pass_string(coilcoolingdxsinglespeed, 0), fault_lvl, coilcoolingdxsinglespeed)
      unless splitsystem_changed
        return false
      end
      # break
    end

    # give an error for the name if no Split System is changed
    return _return_err_message_for_not_unit(runner, existing_coils, coil_choice) unless splitsystem_changed
  ...
  def _check_fault_lvl(runner, coil_choice, fault_lvl)
    ...
    if fault_lvl < 0.0 || fault_lvl > 1.0
      ...
      runner.registerAsNotApplicable("Split SystemLLWithBfOffset is not running for #{coil_choice}. Skipping......")
      return true
    end
    return 'continue'
  end
  ...
  def _write_ems_string(workspace, runner, user_arguments, coil_choice, fault_lvl, coilcoolingdxsinglespeed)
    ...
    unless pass_string(coilcoolingdxsinglespeed, 19).eql?('AirCooled')
      runner.registerError("#{coil_choice} is not air cooled. Impossible to continue in Split SystemLLWithBfOffset. Exiting......")
      return false
    end
  ...
  def _return_err_message_for_not_unit(runner, existing_coils, coil_choice)
    runner.registerError("Measure Split SystemLLWithBfOffset cannot find #{coil_choice}. Exiting......")
...
SplitCAWithSHRChange.new.registerWithApplication

...
$outtmp = 'OutTmp'
$zonename = 'Zone'
$strend = 'forHeatSetptOccChange'
$heatsch_value = 'HeatSetptSch'
$coolsch_value = 'HeatCoolSetptSch'
$occ_value = 'OccCount'
$program_name = 'HeatSetptOffsetbyOutTmp'
$setpt_act_name = 'FinalHeatSetpt'
$program_description_statement = 'heating setpoint shift'
$err_check = false
$allzonechoices = '* All Zones *'

# start the measure
class HeatingThermostatSetpointOccRedByOutdoorTemp < OpenStudio::Ruleset::WorkspaceUserScript
  # human readable name
  def name
    return 'Thermostat heating setpoint manual change by outdoor temperature'
  end

  # human readable description
  def description
    return 'This Measure offsets the thermostat heating setpoint ' \
           'according to the outdoor temperature' \
           ' and to simulate the manual change of thermostat setpoint when ' \
           'outdoor temperature is lower than certain value and the ' \
  ...
  def modeler_description
    return 'To use this Measure, choose the zone to be faulted. Enter the ' \
           'maximum outdoor temperature that the occupant will increase the ' \
  ...
  def arguments(workspace)
    ...
    min_oa_tmp.setDisplayName('Enter the maximum outdoor air temperature (C)')
    min_oa_tmp.setDefaultValue(-10)
    args << min_oa_tmp
  ...
  def run(workspace, runner, user_arguments)
    ...
    if offset_lvl > 0 # only continue if the system is faulted
      ...
      runner.registerAsNotApplicable(
        "#{offset_lvl} in HeatingThermostatSetpointOccRedByOutdoorTemp" \
        ' is smaller than 0K. Skipping......'
      )
    else # not applicable
      runner.registerAsNotApplicable(
        'HeatingThermostatSetpointOccRedByOutdoorTemp is not running ' \
  ...
  def _ems_writer(workspace, runner, zone_choice, min_oa_tmp, offset_lvl)
    ...
    # create strings of EMS sensors to get the schedule value of the heating
    ...
    # create strings of an EMS actuator to override the heating
    # Setpoint of the zone
    _ems_heatingsetpt_actuator(zone_choice, string_objects)
  ...
  def _ems_program_writer(workspace, zone_choice,
    ...
    # This function writes an ems program that changes the heating setpoint in
    # the zone zon_choice depending on the outdoor temperature. If the outdoor
    # air temperature is below min_oa_tmp (in C), it increases the setpoint by
    ...
    string_objects << "
      EnergyManagementSystem:Program,
        ...
        IF OutTmp < MaxOutTmp && OccValue > 0.001, !- Impose the offset when there are people adjusting the thermostat setpt. by outdoor temperature
        SET #{_unique_name_generator(zone_choice, $setpt_act_name)} = HeatOriSetpt+OffSet,
        IF CoolOriSetpt < #{_unique_name_generator(zone_choice, $setpt_act_name)}, !- use the cooling setpoint instead if the heating setpoint becomes too high
  ...
  def _ems_setpt_sch_sensor(workspace, runner, zone_choice, string_objects)
    # This function pushes the ems sensor script for heating setpoint schedule
    # to string_objects. It returns true if the schedule can be found. Otherwise,
    # it returns false.

    # find the Schedule:Year objects related to the zone through
    ...
    unless sch_exist
      runner.registerError("Cannot find #{zone_choice} in "\
                           'HeatingThermostatSetpointOccRedByOutdoorTemp.' \
                           ' Exiting......')
      return false
    end
  ...
  def _fin_thermostat_sch(workspace, thermostatname, schpos = 1)
    ...
    # schedule name. If schpos = 2, it returns the heating setpoint schedule
    # name.
  ...
  def _ems_heatingsetpt_actuator(zone_choice, string_objects)
    # This function pushes an EMS actuator for the zone heating setpoint
...
HeatingThermostatSetpointOccRedByOutdoorTemp.new.registerWithApplication

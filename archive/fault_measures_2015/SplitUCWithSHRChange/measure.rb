...
# start the measure
class SplitUCWithSHRChange < OpenStudio::Ruleset::WorkspaceUserScript
  # human readable name
  def name
    return 'Split System UnderCharging Simulation'
  end
  ...
  def arguments(workspace)
    ...
    # undercharging model
    args = enter_coefficients(args, $q_para_num, "Q_#{$faultnow}", [-1.611900, -1.968100, 2.551400, -0.988030, 0.346750, 0.101090], '')
    args = enter_coefficients(args, $eir_para_num, "EIR_#{$faultnow}", [7.095800, -1.013200, -4.931900, 0.697880, 1.267700, -0.065184], '')
    ...
    bf_para.setDefaultValue(-1.29)  # default change of bypass factor level with fault level in %
    args << bf_para
  ...
  def _check_fault_lvl(runner, coil_choice, fault_lvl)
    ...
    if fault_lvl < 0.0 || fault_lvl > 1.0
      ...
      runner.registerAsNotApplicable("RTULLWithBfOffset is not running for #{coil_choice}. Skipping......")
      return true
    end
    return 'continue'
  end
  ...
  def _write_ems_string(workspace, runner, user_arguments, coil_choice, fault_lvl, coilcoolingdxsinglespeed)
    ...
    unless pass_string(coilcoolingdxsinglespeed, 19).eql?('AirCooled')
      runner.registerError("#{coil_choice} is not air cooled. Impossible to continue in RTULLWithBfOffset. Exiting......")
      return false
    end
...
SplitUCWithSHRChange.new.registerWithApplication

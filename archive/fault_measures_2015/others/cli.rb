...
optparse = OptionParser.new do |opts|
  ...
  opts.on('--server-wait <INTEGER>', 'seconds to wait for job to start before timeout (Default 1800)') do |start_wait|
    options[:start_wait] = start_wait.to_i
  end

  options[:analysis_wait] = 1800
  opts.on('--analysis-wait <INTEGER>', 'seconds to wait for job to complete before timeout (Default 1800)') do |analysis_wait|
...
begin
  ...
  if File.exist?('.temp')
    # Delete redundant files in the folder if it exists
  ...
  # Create another name and try again should error occurs
  begin
    FileUtils.rm_r '.temp'
  rescue
    # Delete files in windows with force deletion
    # It won't crash if it fails in Windows (not tested in other OS)
    FileUtils.rm_rf '.temp', :secure => true
  end
end

...
def main_program_entry(runner, workspace, string_objects, sh_chiller_choice, curve_name, para, model_name, fault_type)
  ...
  # curve in the Chiller:Electric:EIR object with fault the fault model
  #
  # workspace is a WorkSpace object with E+ objects in the building model_name
  #
  # string_objects is an array object storing strings formatted as E+ objects. These objects
...
def fault_adjust_function(runner, workspace, string_objects, fault_type, chillerelectriceir, sh_chiller_choice, model_name, para)
  ...
  string_objects << "
    EnergyManagementSystem:Program,
      ...
      SET TEMP_VAL = C5*Qevap*#{fault_type}FaultLevel, !-<none>
      SET #{fault_type}_FAULT_ADJ_RATIO = #{fault_type}_FAULT_ADJ_RATIO+TEMP_VAL,  !- <none>
      SET TEMP_VAL = C6*#{fault_type}FaultLevel, !-<none>
      SET #{fault_type}_FAULT_ADJ_RATIO = #{fault_type}_FAULT_ADJ_RATIO+TEMP_VAL,  !- <none>
      SET #{fault_type}_FAULT_ADJ_RATIO = 1.0+#{fault_type}_FAULT_ADJ_RATIO,  !- <none>
      ELSE,
      SET #{fault_type}_FAULT_ADJ_RATIO = 1.0, !- <none>
      ENDIF,
    "

...
class CondenserFouling < OpenStudio::Ruleset::WorkspaceUserScript
  ...
  def arguments(workspace)
    ...
    start_time.setDefaultValue(0)  #default is 0am
    args << start_time
  ...
  def _write_ems_string(workspace, runner, user_arguments, coil_choice, fault_lvl, coilcooling, coiltype, time_constant, time_step, start_month, start_date, start_time, end_month, end_date, end_time)
    ...
    if coiltype == 1 #SINGLESPEED
      unless pass_string(coilcooling, 21).eql?('AirCooled')
  ...
  def _write_q_and_eir_curves(workspace, coil_choice, coilcooling, string_objects, runner, coiltype, fault_lvl)
    ...
    ##################################################
    if coiltype == 1 #SINGLESPEED
      string_objects, curve_exist = _write_curves(workspace, coil_choice, coilcooling, string_objects, 'Q', 10, runner, coiltype, [], fault_lvl)
      string_objects, curve_exist = _write_curves(workspace, coil_choice, coilcooling, string_objects, 'EIR', 12, runner, coiltype, [], fault_lvl)

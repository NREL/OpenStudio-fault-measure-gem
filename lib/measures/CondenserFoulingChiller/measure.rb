...
# start the measure
class CondenserFoulingChiller < OpenStudio::Ruleset::WorkspaceUserScript
  # human readable name
  def name
    return 'Condenser Fouling in Chiller'
  end

  # human readable description
  def description
    return 'Following Cheung and Braun (2016), this Measure simulates the effect of condenser fouling fault of water-cooled chillers with shell-and-tube condensers and evaporators to the building performance.'
  end
  ...
  def arguments(workspace)
    ...
    fault_level.setDisplayName('Ratio of water tubes blocked in the condenser at full load.')
    ...
    max_fl.setDefaultValue(0.45) # default maximum level to be overcharged by 40%
    args << max_fl

    # condenser fouling fault model
    args = enter_coefficients(args, $power_para_num, 'power_fault', [-4.278, -0.02031, 0.03281, 0.6404, 0.05028, 0.0], ' for the condenser fouling fault model')

    min_evap_tmp_fault = OpenStudio::Ruleset::OSArgument.makeDoubleArgument('min_evap_tmp_fault', true)
    min_evap_tmp_fault.setDisplayName('Minimum value of evaporator water outlet temperature for the condenser fouling fault model (C)')
    ...
    max_evap_tmp_fault.setDisplayName('Maximum value of evaporator water outlet temperature for the condenser fouling fault model (C)')
    ...
    min_cond_tmp_fault.setDisplayName('Minimum value of condenser inlet temperature for the condenser fouling fault model (C)')
    ...
    max_cond_tmp_fault.setDisplayName('Maximum value of condenser inlet temperature for the condenser fouling fault model (C)')
    ...
    min_cap_fault.setDisplayName('Minimum ratio of evaporator heat transfer rate to the reference capacity for the condenser fouling fault model')
    min_cap_fault.setDefaultValue(0.268)
    args << min_cap_fault

    max_cap_fault = OpenStudio::Ruleset::OSArgument.makeDoubleArgument('max_cap_fault', true)
    max_cap_fault.setDisplayName('Maximum ratio of evaporator heat transfer rate to the reference capacity for the condenser fouling fault model')
  ...
  def run(workspace, runner, user_arguments)
    ...
    runner.registerAsNotApplicable("CondenserFoulingChiller is not running for #{chiller_choice}. Skipping......")
    return true
  end
  ...
  def error_msg_for_not_rtu(runner, chiller_choice, existing_coils)
    ...
    runner.registerError("Measure CondenserFoulingChiller cannot find #{chiller_choice}. Exiting......")
...
CondenserFoulingChiller.new.registerWithApplication

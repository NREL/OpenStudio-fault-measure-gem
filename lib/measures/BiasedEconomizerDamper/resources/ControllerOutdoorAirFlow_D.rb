...
def econ_damper_bias_ems_main_body(runner, workspace, controlleroutdoorair, pos_bias, oacontrollername)

  #workspace is the Workspace object in EnergyPlus Measure script
  #this function will return an empty string for all other inputs
  ...
  main_body = "
    EnergyManagementSystem:Program,
      bias_#{econ_short_name}_#{$faulttype}, !- Name
      ...
      SET MIX_FLOW_GB#{econ_short_name}_#{$faulttype} = "+econ_short_name+"MixAirFlow_CTRL_#{$faulttype}, !- <none>
  ...
  main_body = main_body+"
    ...
    SET OATmp = "+econ_short_name+"OATTemp1_#{$faulttype}, !- <none>
  ...
  main_body = main_body+"
    ...
    SET HMDOT_D = HMDesAirflow"+econ_short_name+"_#{$faulttype}, !- <none>
    SET MDOT_DES = @Max CMDOT_D HMDOT_D, !- <none>
    SET MDOT_OA_MIN = MinOAMdot"+econ_short_name+"_#{$faulttype}, !- <none>
    SET MDOT_OA_MAX = MaxOAMdot"+econ_short_name+"_#{$faulttype}, !- <none>
          ...
          outdoorairspecs.each do |outdoorairspec|
            if controllermechventilation.getString(4+3*i+2).to_s.eql?(outdoorairspec.getString(0).to_s)
              ...
              if outdoorairspec.getString(1).to_s.eql?("Sum") #add code for summation
                ...
                #TODO:
                #there is still slight difference in minimum outdoor air flow rate calculation between baseline (without fault model) model and faulted (fault model with FI = 0) model. 
                #####################################################
                if peoples.empty?
                  main_body = main_body+"
                            SET ZONE_PPL = 0, !- <none>
                  ...
                  peoples.each do |people|
                    if people.getString(1).to_s.eql?(controllermechventilation.getString(4+3*i+1).to_s)
                      main_body = main_body+"
                        SET ZONE_PPL = "+zone_name_new+"_PEOPLE_#{$faulttype}, !- NEED SENSOR FOR ZONE People Occupant Count
                      "
                    else
                      zonelists.each do |zonelist|
                        if people.getString(1).to_s.eql?(zonelist.getString(0).to_s)
                          for i in 1..zonelist.numFields-1  #each zone
  ...
  main_body = main_body+"
    ...
    SET OA_SIGN = (RETTmp-"+econ_short_name+"MASetPoint1_#{$faulttype})/(RETTmp-OATmp_ORI), !- Initialize the signal
    ELSE, !- <none>
    IF RETTmp < "+econ_short_name+"MASetPoint1_#{$faulttype} && RETTmp >= OATmp_ORI, !- <none>
    SET OA_SIGN = -1, !- <none>
    ELSEIF RETTmp < "+econ_short_name+"MASetPoint1_#{$faulttype} && RETTmp < OATmp_ORI, !- <none>
    SET OA_SIGN = 1, !- <none>
    ELSEIF RETTmp >= "+econ_short_name+"MASetPoint1_#{$faulttype} && RETTmp >= OATmp_ORI,
    SET OA_SIGN = 1, !- <none>
    ELSEIF RETTmp >= "+econ_short_name+"MASetPoint1_#{$faulttype} && RETTmp < OATmp_ORI,
    SET OA_SIGN = -1, !- <none>
    ENDIF, !- <none>
  ...
  if controlleroutdoorair.getString(7).to_s.eql?("NoEconomizer")
    ...
    main_body = main_body+"
      ...
      SET ECON_OP = True, !- <none>
      IF OATmp_ORI > "+econ_short_name+"MASetPoint1_#{$faulttype}, !- <none>
      SET OA_SIGN = 1, !- <none>
      ENDIF, !- <none>
    "
    ...
    if controlleroutdoorair.getString(21).to_s.eql?("Yes")  #High humidity control check
      main_body = main_body+"
        IF ZoneHumidLOAD"+econ_short_name+"_#{$faulttype} < 0.0, !- High Humidity Control
      ...
      if controlleroutdoorair.getString(24).to_s.eql?("Yes")  #Control High Indoor Humidity Based on Outdoor Humidity Ratio
        main_body = main_body+"
          IF ZoneHumid"+econ_short_name+"_#{$faulttype} <= OAHumRat, !- Control High Indoor Humidity Based on Outdoor Humidity Ratio
    ...
    if not controlleroutdoorair.getString(20).to_s.eql?("")  #Time of Day Economizer Control Schedule Name
      main_body = main_body+"
        SET ECON_FLOW_SCH_VAL = ECONCTRL"+econ_short_name+"_SCH_#{$faulttype}, !- <none>
  ...
  main_body = main_body+"
    ...
    SET MIXTEMPSET_GB#{econ_short_name}_#{$faulttype} = "+econ_short_name+"MASetPoint1_#{$faulttype}, !-<none>
  ...
  if controlleroutdoorair.getString(15).to_s.eql?("FixedMinimum")
    main_body = main_body+"
      ...
      SET DUMMY = MinOAMdot"+econ_short_name+"_#{$faulttype}, !- Name too long
  ...
  main_body = main_body+"
    ...
    SET FinalFlow = OA_NEW, !- <none>
    ENDIF, !- <none>
    SET "+econ_short_name+"MDOT_OA_#{$faulttype} = FinalFlow; !- <none>
  "
...
def check_setpoints(workspace, controlleroutdoorair)
  ...
  if not controlleroutdoorair.getString(11).to_s.eql?("")  #Economizer Maximum Limit Dewpoint Temperature
    main_body = main_body+"
      SET OATdew = @TdpFnWPb OAHumRat PTmp, !- <none>
...
def econ_damper_bias_ems_other(runner, string_objects, workspace, controlleroutdoorair)
  ...
  #bias_str is a string: "RET" means return air sensor and "OA" means an outdoor
  ...
  string_objects << "
    EnergyManagementSystem:ProgramCallingManager,
      EMSCallt_bias"+econ_short_name+"_#{$faulttype}, !- Name
  ...
  string_objects << "   
    EnergyManagementSystem:Actuator,
      "+econ_short_name+"MDOT_OA_#{$faulttype},        !- Name
  ...
  string_objects << "
    EnergyManagementSystem:InternalVariable,
      DesAirflow"+econ_short_name+"_#{$faulttype},
      "+airsystem_name+",
  ...
  string_objects << "
    EnergyManagementSystem:InternalVariable,
      CMDesAirflow"+econ_short_name+"_#{$faulttype},
      "+airsystem_name+",
  ...
  string_objects << "
    EnergyManagementSystem:InternalVariable,
      HMDesAirflow"+econ_short_name+"_#{$faulttype},
      "+airsystem_name+",
  ...
  string_objects << "
    EnergyManagementSystem:InternalVariable,
      MinOAMdot"+econ_short_name+"_#{$faulttype},
      "+econ_choice+",
  ...
  string_objects << "
    EnergyManagementSystem:InternalVariable,
      MaxOAMdot"+econ_short_name+"_#{$faulttype},
      "+econ_choice+",
                    ...
                    if people.getString(1).to_s.eql?(zonelist.getString(0).to_s)
                      for i in 1..zonelist.numFields-1  #for each zone
                        ...
                        if zone_name_inlist.eql?(zone_name)
                          ...
                          #NOTE: "People Count Design Level" does not work iff "ZoneList" is defined instead of "Zone"
                          string_objects << "
                            EnergyManagementSystem:Sensor,
                            "+zone_name_new+"_PEOPLE_#{$faulttype}, !- Name
  ...
  string_objects << "
    EnergyManagementSystem:Sensor,
      "+econ_short_name+"RETTemp1_#{$faulttype},  !- Name
  ...
  string_objects << "
    EnergyManagementSystem:Sensor,
      "+econ_short_name+"RETOmega1_#{$faulttype},  !- Name
      "+ret_node_name+",                        !- Output:Variable or Output:Meter Index Key Name
  ...
  string_objects << "
    EnergyManagementSystem:Sensor,
      "+econ_short_name+"RETPressure1_#{$faulttype},  !- Name
      "+ret_node_name+",                        !- Output:Variable or Output:Meter Index Key Name
  ...
  string_objects << "
    EnergyManagementSystem:Sensor,
      "+econ_short_name+"MASetPoint1_#{$faulttype},  !- Name
  ...
  string_objects << "
    EnergyManagementSystem:Sensor,
      "+econ_short_name+"OATTemp1_#{$faulttype},  !- Name
  ...
  string_objects << "
    EnergyManagementSystem:Sensor,
      "+econ_short_name+"OATOmega1_#{$faulttype},  !- Name
      "+oa_node_name+",                        !- Output:Variable or Output:Meter Index Key Name
      System Node Humidity Ratio;                !- Output:Variable or Output:Meter Name
  "
  ...
  string_objects << "
    EnergyManagementSystem:Sensor,
      "+econ_short_name+"MixAirFlow_CTRL_#{$faulttype},  !- Name
  ...
  if controlleroutdoorair.getString(21).to_s.eql?("Yes")
    ...
    string_objects << "
      EnergyManagementSystem:Sensor,
        ZoneHumid"+econ_short_name+"_#{$faulttype},  !- Name
    ...
    string_objects << "
      EnergyManagementSystem:Sensor,
        ZoneHumidLOAD"+econ_short_name+"_#{$faulttype},  !- Name
        "+humidistat_zone+",                        !- Output:Variable or Output:Meter Index Key Name
  ...
  if not controlleroutdoorair.getString(20).to_s.eql?("")
    string_objects << "
      EnergyManagementSystem:Sensor,
        ECONCTRL"+econ_short_name+"_SCH_#{$faulttype}, !- Name

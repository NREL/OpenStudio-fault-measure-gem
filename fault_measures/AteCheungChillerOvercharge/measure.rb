...
# start the measure
class AteCheungChillerOvercharge < OpenStudio::Ruleset::WorkspaceUserScript
  # human readable name
  def name
    return 'Water-cooled chiller overcharged (Cheung and Braun 2016)'
  end

  # human readable description
  def description
    return 'Following Cheung and Braun (2016), this Measure simulates the effect of overcharging of water-cooled chillers with shell-and-tube condensers and evaporators to the building performance.'
  end
  ...
  def arguments(workspace)
    ...
    # overcharging model
    args = enter_coefficients(args, $power_para_num, 'power_fault', [-8.854, -0.01228, 0.04126, 0.2838, 0.1807, 0.0], ' for the overcharging model')
    ...
    min_cap_fault.setDefaultValue(0.277)
    args << min_cap_fault
  ...
  def run(workspace, runner, user_arguments)
    ...
    runner.registerAsNotApplicable("AteCheungChillerOvercharge is not running for #{chiller_choice}. Skipping......")
    return true
  end
  ...
  def error_msg_for_not_rtu(runner, chiller_choice, existing_coils)
    ...
    runner.registerError("Measure AteCheungChillerOvercharge cannot find #{chiller_choice}. Exiting......")
...
AteCheungChillerOvercharge.new.registerWithApplication

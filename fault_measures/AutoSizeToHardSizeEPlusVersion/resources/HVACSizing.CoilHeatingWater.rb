...
class OpenStudio::Model::CoilHeatingWater
  ...
  def autosize
    self.autosizeMaximumWaterFlowRate
    self.autosizeUFactorTimesAreaValue
    self.autosizeRatedCapacity
  end
  ...
  def applySizingValues
    ...
    if maximum_water_flow_rate.is_initialized and self.isMaximumWaterFlowRateAutosized
      self.setMaximumWaterFlowRate(maximum_water_flow_rate.get) 
    end
    
    u_factor_times_area_value = self.autosizedUFactorTimesAreaValue
    if u_factor_times_area_value.is_initialized and self.isUFactorTimesAreaValueAutosized
      self.setUFactorTimesAreaValue(u_factor_times_area_value.get)
    end
    
    rated_capacity = self.autosizedRatedCapacity
    if rated_capacity.is_initialized and self.isRatedCapacityAutosized

...
class SplitCondenserFanMotorEfficiencyFault < OpenStudio::Ruleset::WorkspaceUserScript
  ...
  # human readable name
  def name
    return "Split System Condenser Fan Motor Efficiency Fault"
  end

  # human readable description
  def description
    return "This Measure simulates the effect of fan motor efficiency fault at condenser of a split system or DX unit to the building performance."
  end

  # human readable description of workspace approach
  def modeler_description
    return "To use this Measure, choose the Coil:Cooling:DX:SingleSpeed object to be faulted and a schedule of fault level. Define the fault level as the degradation level of fan motor efficiency. If the fault level is outside the range of zero and one, an error will occur. The model is only applicable for systems with rated capacity between 5kW to 70kW."
  end
  ...
  def arguments(workspace)
    ...
    degrd_lvl.setDefaultValue(0.2)  #default fouling level to be 50%
    args << degrd_lvl
    ...
    fan_power_ratio.setDefaultValue(0.052671882)  #defaulted calcualted to be 0.06127853
  ...
  def run(workspace, runner, user_arguments)
    ...
    if (schedule_exist || degrd_lvl != 0) # only continue if the user is running the module
      ...
      #find the SPLIT to change
      no_SPLIT_changed = true
      existing_coils = []
...
SplitCondenserFanMotorEfficiencyFault.new.registerWithApplication

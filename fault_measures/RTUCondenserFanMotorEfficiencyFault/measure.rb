...
class RTUCondenserFanMotorEfficiencyFault < OpenStudio::Ruleset::WorkspaceUserScript
  ...
  def run(workspace, runner, user_arguments)
    ...
    if (schedule_exist || degrd_lvl != 0) # only continue if the user is running the module
      ...
      if no_RTU_changed
        runner.registerError("Measure RTUCondenserFanMotorEfficiencyFault cannot find "+coil_choice+". Exiting......")
      ...
      runner.registerAsNotApplicable("RTUCondenserFanMotorEfficiencyFault is not running for "+coil_choice+". Skipping......")
    end

...
require "#{File.dirname(__FILE__)}/resources/nosqfaultcalculationchillerelectriceir"
...
# start the measure
class ChillerCondenserFouling < OpenStudio::Ruleset::WorkspaceUserScript
  # human readable name
  def name
    return 'Water-cooled chiller with fouled condenser'
  end

  # human readable description
  def description
    return 'This Measure simulates the effect of condenser fouling of water-cooled chillers with shell-and-tube condensers and evaporators to the building performance.'
  end

  # human readable description of workspace approach
  def modeler_description
    return 'To use this Measure, choose the Chiller:Electric:EIR object to be faulted and a schedule of fault level. Define the fault level as the ratio of blocked tubes on the water-side of the condenser to the total number of tubes in the condenser. If the fault level is outside the range of zero and one, an error will occur.'
  end
  ...
  def arguments(workspace)
    ...
    fault_level.setDefaultValue(0.1)  # default fault level to be overcharged by 10%
    args << fault_level

    # fault level limits
    max_fl = OpenStudio::Ruleset::OSArgument.makeDoubleArgument('max_fl', true)
    max_fl.setDisplayName('Maximum value of fault level')
    max_fl.setDefaultValue(0.45) # default maximum level to be overcharged by 45%
    args << max_fl

    # overcharging model
    args = enter_coefficients(args, $power_para_num, 'power_fault', [-3.475812754, -0.014398372, 0.0250766, 0.738057225, 0.115719097], ' for the condenser fouling model')

    min_evap_tmp_fault = OpenStudio::Ruleset::OSArgument.makeDoubleArgument('min_evap_tmp_fault', true)
    min_evap_tmp_fault.setDisplayName('Minimum value of evaporator water outlet temperature for the overcharging model (C)')
    min_evap_tmp_fault.setDefaultValue(4.03)
    args << min_evap_tmp_fault

    max_evap_tmp_fault = OpenStudio::Ruleset::OSArgument.makeDoubleArgument('max_evap_tmp_fault', true)
    max_evap_tmp_fault.setDisplayName('Maximum value of evaporator water outlet temperature for the overcharging model (C)')
    max_evap_tmp_fault.setDefaultValue(10.4)
    args << max_evap_tmp_fault

    min_cond_tmp_fault = OpenStudio::Ruleset::OSArgument.makeDoubleArgument('min_cond_tmp_fault', true)
    min_cond_tmp_fault.setDisplayName('Minimum value of condenser inlet temperature for the overcharging model (C)')
    ...
    max_cond_tmp_fault.setDisplayName('Maximum value of condenser inlet temperature for the overcharging model (C)')
    ...
    min_cap_fault.setDisplayName('Minimum ratio of evaporator heat transfer rate to the reference capacity for the overcharging model')
    min_cap_fault.setDefaultValue(0.27)
    args << min_cap_fault

    max_cap_fault = OpenStudio::Ruleset::OSArgument.makeDoubleArgument('max_cap_fault', true)
    max_cap_fault.setDisplayName('Maximum ratio of evaporator heat transfer rate to the reference capacity for the overcharging model')
  ...
  def run(workspace, runner, user_arguments)
    ...
    runner.registerAsNotApplicable("ChillerCondenserFouling is not running for #{chiller_choice}. Skipping......")
    return true
  end
  ...
  def check_schedule_or_lvl_error(workspace, runner, sch_choice, schedule_exist, fault_level)
    ...
    if schedule_exist
      ...
      if fault_level > 1.0 || fault_level < 0.0
        runner.registerError("Fault level #{fault_level} in ChillerCondenserFouling is outside the range of 0 to 1. Exiting......")
        return scheduletypelimits, true
      end
    end
  ...
  def check_ems_program_caller_exist(workspace, sh_chiller_choice)
    ...
    ems_callers.each do |ems_caller|
      return true if pass_string(ems_caller, 0).to_s.eql?("EMSCallChillerElectricEIRDegradation#{sh_chiller_choice}power")
    end
    return false
  end
  ...
  def error_msg_for_not_rtu(runner, chiller_choice, existing_coils)
    ...
    runner.registerError("Measure ChillerCondenserFouling cannot find #{chiller_choice}. Exiting......")
...
ChillerCondenserFouling.new.registerWithApplication
